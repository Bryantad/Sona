...
def handle_types_status(args) -> int:
    ...
    try:
        ...
        print(f"Effective mode: {config.get_effective_mode().value}")
        
        # Determine source of configuration
        if config._cli_mode is not None:
            cli_val = config._cli_mode.value
            print(f"Source: CLI argument (--types={cli_val})")
        elif config._env_mode is not None:
            env_val = config._env_mode.value
            print(f"Source: Environment variable (SONA_TYPES={env_val})")
        elif config._config_mode is not None:
            cfg_val = config._config_mode.value
        ...
        if excludes:
            print(f"Exclusion patterns: {excludes}")
        else:
            print("Exclusion patterns: None")
        
        # Show other settings
        print(f"Type checking enabled: {config.should_check_types()}")
        print(f"Fail on errors: {config.should_fail_on_error()}")
        print(f"Emit warnings: {config.should_emit_warnings()}")
        
        # Show log sink (to be implemented)
        print("Log sink: stderr (JSONL)")
        
        return 0
        
    except Exception as e:
        print(f"❌ Error checking type configuration: {e}")
        return 1
