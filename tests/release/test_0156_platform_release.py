...
from tools.release import platform_release_0156 as release

ROOT = Path(__file__).resolve().parents[2]


def test_0156_platform_release_contract_uses_current_artifact_names() -> None:
    assert release.VERSION == "0.15.6"
    assert "sona-native-0.15.6-windows-x86_64.zip" in release.NATIVE_ASSETS
    assert "sona-native-0.15.6-linux-x86_64-musl.tar.gz" in release.NATIVE_ASSETS
    assert "sona-native-0.15.6-linux-aarch64-musl.tar.gz" in release.NATIVE_ASSETS
    assert "sona-native-0.15.6-macos-x86_64.tar.gz" in release.NATIVE_ASSETS
    assert "sona-native-0.15.6-macos-aarch64.tar.gz" in release.NATIVE_ASSETS
...
def test_0156_workflow_uses_matching_tool_and_host_matrix() -> None:
    workflow = (
        ROOT / ".github" / "workflows" / "release-platforms-0156.yml"
    ).read_text(encoding="utf-8")

    assert "Sona 0.15.6 cross-platform release candidates" in workflow
    assert "tools/release/platform_release_0156.py" in workflow
    assert "sona-ai-native-programming-0.15.6.vsix" in workflow
    for fragment in (
        "windows-2025",
        "ubuntu-24.04",
        "ubuntu-24.04-arm",
        ...
        "aarch64-apple-ios",
        "platform_release_0156.py verify",
    ):
