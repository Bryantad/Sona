...
class FocusTimerState {
    ...
    onTick;
    onPhaseComplete;
    constructor(statusBarItem, context, onTick, onPhaseComplete) {
    ...
    loadState() {
        ...
        const savedDate = this.context.globalState.get('focusTimer.lastDate');
        if (savedDate === today) {
            this.todayFocusSessions = this.context.globalState.get('focusTimer.todaySessions', 0);
            this.todayFocusMinutes = this.context.globalState.get('focusTimer.todayMinutes', 0);
            this.currentStreak = this.context.globalState.get('focusTimer.streak', 0);
        }
        else {
            // New day - check if streak continues
            const lastStreak = this.context.globalState.get('focusTimer.streak', 0);
            const lastGoalReached = this.context.globalState.get('focusTimer.lastGoalReached', false);
    ...
    startFocus() {
        const config = vscode.workspace.getConfiguration('focusTimer');
        const duration = config.get('focusDuration', 25);
        this.phase = 'focus';
    ...
    startShortBreak() {
        const config = vscode.workspace.getConfiguration('focusTimer');
        const duration = config.get('shortBreakDuration', 5);
    ...
    startLongBreak() {
        const config = vscode.workspace.getConfiguration('focusTimer');
        const duration = config.get('longBreakDuration', 15);
    ...
    logSession(phase, completed) {
        const config = vscode.workspace.getConfiguration('focusTimer');
        if (!config.get('logSessionsToFile', true)) {
            return;
        }
...
// STATUS BAR
// ============================================================================
function getPhaseIcon(phase, isPaused) {
    if (isPaused)
        return '$(debug-pause)';
    switch (phase) {
...
function updateStatusBar(statusBar, state) {
    const icon = getPhaseIcon(state.phase, state.isPaused);
    if (state.phase === 'idle') {
        statusBar.text = `${icon} Focus Timer`;
        statusBar.tooltip = new vscode.MarkdownString(`**Focus Timer**\n\n` +
        ...
        statusBar.tooltip = new vscode.MarkdownString(`**${label}**${pauseIndicator}\n\n` +
            `Progress: ${progress}%\n\n` +
...
// NOTIFICATIONS
// ============================================================================
async function showPhaseCompleteNotification(phase, state, onNext) {
    const config = vscode.workspace.getConfiguration('focusTimer');
    if (!config.get('showNotifications', true)) {
        return;
    }
    const sessionsBeforeLong = config.get('sessionsBeforeLongBreak', 4);
    if (phase === 'focus') {
        const isLongBreak = state.sessionCount % sessionsBeforeLong === 0;
        const breakType = isLongBreak ? 'long break' : 'short break';
        const breakDuration = isLongBreak
            ? config.get('longBreakDuration', 15)
            : config.get('shortBreakDuration', 5);
        const message = `🎉 Focus session complete! Time for a ${breakType} (${breakDuration} min).`;
        if (config.get('autoStartBreaks', true)) {
            vscode.window.showInformationMessage(message);
            onNext(isLongBreak ? 'longBreak' : 'shortBreak');
        }
        else {
            const action = await vscode.window.showInformationMessage(message, 'Start Break', 'Skip');
        ...
        if (config.get('autoStartFocus', false)) {
            vscode.window.showInformationMessage(message);
            onNext('focus');
        }
        else {
            const action = await vscode.window.showInformationMessage(message, 'Start Focus', 'Skip');
...
// STATS PANEL
// ============================================================================
async function showStatsPanel(context, state) {
    const config = vscode.workspace.getConfiguration('focusTimer');
    const dailyGoal = config.get('dailyGoal', 8);
    const progress = Math.min(100, Math.round((state.todayFocusSessions / dailyGoal) * 100));
    const progressBar = '█'.repeat(Math.floor(progress / 10)) + '░'.repeat(10 - Math.floor(progress / 10));
    const panel = vscode.window.createWebviewPanel('focusTimerStats', 'Focus Timer Stats', vscode.ViewColumn.One, { enableScripts: true });
...
async function showHistoryPanel() {
    ...
    try {
        ...
        const panel = vscode.window.createWebviewPanel('focusTimerHistory', 'Focus Timer History', vscode.ViewColumn.One, {});
        panel.webview.html = `<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
...
function activate(context) {
    ...
    // Initialize state
    const state = new FocusTimerState(statusBarItem, context, () => updateStatusBar(statusBarItem, state), (phase) => {
        ...
        showPhaseCompleteNotification(phase, state, (action) => {
            switch (action) {
                case 'focus':
                    ...
                    break;
                case 'shortBreak':
                    state.startShortBreak();
                    break;
                case 'longBreak':
                    state.startLongBreak();
                    break;
            }
            updateStatusBar(statusBarItem, state);
        });
    ...
    updateStatusBar(statusBarItem, state);
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('focusTimer.start', () => {
        state.startFocus();
        updateStatusBar(statusBarItem, state);
        vscode.window.showInformationMessage('🔥 Focus session started! Stay focused.');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('focusTimer.pause', () => {
        if (state.isPaused) {
            state.resume();
    ...
    context.subscriptions.push(vscode.commands.registerCommand('focusTimer.stop', () => {
        state.stop();
        updateStatusBar(statusBarItem, state);
        vscode.window.showInformationMessage('⏹️ Session stopped');
    }));
    context.subscriptions.push(vscode.commands.registerCommand('focusTimer.skip', () => {
        state.skip();
        updateStatusBar(statusBarItem, state);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('focusTimer.showStats', () => {
        showStatsPanel(context, state);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('focusTimer.showHistory', () => {
        showHistoryPanel();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('focusTimer.resetStreak', async () => {
        const confirm = await vscode.window.showWarningMessage('Reset your streak to 0?', { modal: true }, 'Reset');
        if (confirm === 'Reset') {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('focusTimer.configure', () => {
