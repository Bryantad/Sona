...
function estimateBraceNesting(text: string): number {
  let depth = 0;
  let maxDepth = 0;
  let inSingle = false;
  let inDouble = false;
...
function estimateIndentNesting(document: vscode.TextDocument, maxLines: number): number {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const editHistory = new Map<string, EditEvent[]>();

  function getConfig() {
    const cfg = vscode.workspace.getConfiguration('cognitiveLoadMeter');
    return {
      enabled: cfg.get<boolean>('enabled', true),
      lookbackMinutes: cfg.get<number>('lookbackMinutes', 10),
      debounceMs: cfg.get<number>('debounceMs', 350),
      maxLinesForNesting: cfg.get<number>('maxLinesForNesting', 2000)
    };
  }

  function pruneEdits(events: EditEvent[], cutoff: number): EditEvent[] {
  ...
  function computeLoad(document: vscode.TextDocument) {
    const cfg = getConfig();
    const now = Date.now();
    ...
    const uniqueLineBuckets = new Set<number>();
    const regionBuckets = new Set<number>();
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cognitiveLoadMeter.toggleEnabled', async () => {
      const cfg = vscode.workspace.getConfiguration('cognitiveLoadMeter');
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cognitiveLoadMeter.showDetails', async () => {
      const doc = getActiveDocument();
      if (!doc) return;
      const info = computeLoad(doc);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cognitiveLoadMeter.resetHistory', async () => {
      editHistory.clear();
      updateStatus();
    })
  );
