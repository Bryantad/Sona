 ...
 */
export function detectReceiptKind(raw: Record<string, unknown>): ReceiptKind {
...
function extractBase(raw: Record<string, unknown>): BaseReceipt {
    ...
    return {
        ...
        redaction: redactionSource
            ? {
                  profile: asString(redactionSource.profile, undefined as unknown as string),
                  mode: asString(redactionSource.mode, undefined as unknown as string),
                  token_strategy: asString(redactionSource.token_strategy, undefined as unknown as string)
              }
            : undefined,
        signature: signatureSource
            ? {
                  ...
                  key_id: asString(signatureSource.key_id, undefined as unknown as string),
                  signed_at_utc: asString(signatureSource.signed_at_utc, undefined as unknown as string)
              }
            : undefined
    };
}
...
function parseExecutionReceipt(raw: Record<string, unknown>): ExecutionReceipt | undefined {
    ...
    return {
        ...
        dependencies: {
            lockfile_sha256: dependencies.lockfile_sha256 === null ? null : asString(dependencies.lockfile_sha256, null as unknown as string),
...
function parseDirectoryReceipt(raw: Record<string, unknown>): DirectoryReceipt | undefined {
...
export function parseReceipt(content: string): ParsedReceipt | undefined {
    let parsed: unknown;
    try {
