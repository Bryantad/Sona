...
exports.ReceiptTreeDataProvider = exports.ReceiptTreeItem = void 0;
...
function describeSignatureShort(parsed) {
...
class ReceiptTreeItem extends vscode.TreeItem {
    ...
    isGroup;
    groupKey;
    constructor(label, collapsibleState, receiptFile, isGroup = false, groupKey) {
    ...
    buildTooltip(parsed) {
        ...
        if (parsed.kind === 'execution') {
            const git = parsed.data.git;
            if (git) {
                md.appendMarkdown(`- **Git**: ${git.branch}@${git.commit.slice(0, 7)}${git.dirty ? ' (dirty)' : ''}\n`);
            }
        }
        if (parsed.timestamp) {
            md.appendMarkdown(`\n*${parsed.timestamp}*`);
    ...
    buildDescription(parsed) {
        ...
        let duration = '';
        if (parsed.kind === 'execution') {
            duration = `${parsed.data.execution.duration_ms}ms`;
        }
        const parts = [status];
        if (duration) {
            parts.push(duration);
    ...
    getIcon(parsed) {
        switch (parsed.kind) {
            case 'execution': {
                const exitCode = parsed.data.execution.exit_code;
                return exitCode === 0
...
exports.ReceiptTreeItem = ReceiptTreeItem;
// ---------------------------------------------------------------------------
// Tree data provider
// ---------------------------------------------------------------------------
class ReceiptTreeDataProvider {
    output;
    ...
    getConfig() {
        const config = vscode.workspace.getConfiguration('sonaReceipts');
        return {
            maxReceipts: Math.max(1, config.get('maxReceipts', 100)),
            groupByDate: config.get('groupByDate', true),
            autoRefresh: config.get('autoRefresh', true),
            receiptGlob: config.get('receiptGlob', '**/*.receipt.json') || '**/*.receipt.json',
            statusFilter: config.get('statusFilter', 'all') || 'all'
        };
    }
    setupWatcher() {
    ...
    async getChildren(element) {
        ...
        return this.receipts
            ...
            .map((item) => new ReceiptTreeItem(item.parsed.displayLabel, vscode.TreeItemCollapsibleState.None, item));
    }
    async loadReceipts() {
        ...
        for (const uri of files) {
            try {
                ...
                const parsed = (0, parse_1.parseReceipt)(Buffer.from(content).toString('utf8'));
                if (!parsed) {
                    invalidCount += 1;
                    continue;
                }
    ...
    getGroupedItems() {
        ...
        return [...groups.entries()]
            ...
            .map(([key, group]) => {
            const item = new ReceiptTreeItem(group.label, vscode.TreeItemCollapsibleState.Expanded, undefined, true, key);
            item.description = `${group.count} receipt${group.count === 1 ? '' : 's'}`;
            return item;
        });
    }
    getFlatItems() {
        return this.receipts.map((item) => new ReceiptTreeItem(item.parsed.displayLabel, vscode.TreeItemCollapsibleState.None, item));
    }
    getTreeItem(element) {
        return element;
    }
...
exports.ReceiptTreeDataProvider = ReceiptTreeDataProvider;
