...
exports.describeSignature = describeSignature;
exports.describeRedaction = describeRedaction;
exports.renderExecutionReceipt = renderExecutionReceipt;
exports.renderDirectoryReceipt = renderDirectoryReceipt;
exports.renderUnknownReceipt = renderUnknownReceipt;
exports.formatReceiptForDiff = formatReceiptForDiff;
...
function describeSignature(data) {
    if (!data.signature) {
        return 'unsigned';
    }
...
function describeRedaction(data) {
    if (!data.redaction) {
        return 'none';
    }
    const parts = [data.redaction.profile, data.redaction.mode, data.redaction.token_strategy].filter((v) => Boolean(v));
...
function renderGovernance(data) {
    const rows = [
        ...
        .map(([label, value]) => `<span class="label">${escapeHtml(label)}</span><span class="value">${escapeHtml(String(value))}</span>`)
        .join('');
    if (!rows) {
        return '';
    }
...
function renderMetadata(parsed, filePath) {
...
function renderExecutionReceipt(receiptFile) {
    const r = receiptFile.parsed.data;
...
function renderDirectoryReceipt(receiptFile) {
    const r = receiptFile.parsed.data;
    const entryRows = r.entries
        .slice(0, 200)
        .map((e) => `<tr><td>${escapeHtml(e.path)}</td><td>${escapeHtml(e.sha256.slice(0, 12))}…</td><td>${e.size_bytes}</td></tr>`)
        .join('');
    const body = `
    <div class="header">
...
function renderUnknownReceipt(receiptFile) {
...
class ReceiptDetailPanel {
    ...
    static show(receipt) {
        ...
        const panel = vscode.window.createWebviewPanel('sonaReceiptDetail', `Receipt: ${receipt.parsed.displayLabel}`, column ?? vscode.ViewColumn.One, { enableScripts: false });
...
// Diff formatter
// ---------------------------------------------------------------------------
function formatReceiptForDiff(parsed) {
    const lines = [];
