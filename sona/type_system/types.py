...
class Type(ABC):
    ...
    def __init__(self, kind: TypeKind, location: Optional[SourceLocation] = None):
        self.kind = kind
...
class PrimitiveType(Type):
    ...
    def __init__(self, name: str, location: Optional[SourceLocation] = None):
        super().__init__(TypeKind.PRIMITIVE, location)
...
class TypeVariable(Type):
    ...
    def __init__(self, name: Optional[str] = None, location: Optional[SourceLocation] = None):
...
class FunctionType(Type):
    ...
    def __str__(self) -> str:
        ...
        elif len(self.param_types) == 1:
            return f"{self.param_types[0]} -> {self.return_type}"
        else:
    ...
    def free_variables(self) -> Set[str]:
        ...
        for param_type in self.param_types:
            vars_set.update(param_type.free_variables())
        vars_set.update(self.return_type.free_variables())
        return vars_set

    def substitute(self, substitution: Dict[str, Type]) -> Type:
        new_param_types = [t.substitute(substitution) for t in self.param_types]
...
class GenericType(Type):
    ...
    def __init__(self, constructor: str, type_args: List[Type],
                 location: Optional[SourceLocation] = None):
    ...
    def free_variables(self) -> Set[str]:
        ...
        for type_arg in self.type_args:
            vars_set.update(type_arg.free_variables())
        return vars_set

    def substitute(self, substitution: Dict[str, Type]) -> Type:
        new_type_args = [t.substitute(substitution) for t in self.type_args]
...
class TupleType(Type):
    ...
    def free_variables(self) -> Set[str]:
        ...
        for element_type in self.element_types:
            vars_set.update(element_type.free_variables())
        return vars_set

    def substitute(self, substitution: Dict[str, Type]) -> Type:
        new_element_types = [t.substitute(substitution) for t in self.element_types]
...
class RecordType(Type):
    ...
    def __str__(self) -> str:
        if not self.fields:
            return "{}"
        field_strs = [f"{name}: {type_}" for name, type_ in sorted(self.fields.items())]
        return "{" + ", ".join(field_strs) + "}"

    def free_variables(self) -> Set[str]:
        vars_set = set()
        for field_type in self.fields.values():
            vars_set.update(field_type.free_variables())
        return vars_set

    def substitute(self, substitution: Dict[str, Type]) -> Type:
        new_fields = {name: type_.substitute(substitution)
...
def list_type(element_type: Type) -> GenericType:
    """Create List[T] type"""
    return GenericType("List", [element_type])

def dict_type(key_type: Type, value_type: Type) -> GenericType:
    """Create Dict[K, V] type"""
    return GenericType("Dict", [key_type, value_type])

def optional_type(inner_type: Type) -> GenericType:
    """Create Optional[T] type"""
    return GenericType("Optional", [inner_type])


class TypeScheme:
    """Type scheme for polymorphic types: ∀ type_vars. type"""
    ...
    def instantiate(self) -> Type:
        """Create a fresh instance of this type scheme"""
        if not self.type_vars:
            return self.type
        ...
        for var in self.type_vars:
            substitution[var] = TypeVariable()

        return self.type.substitute(substitution)

    def generalize(self, env_free_vars: Set[str]) -> 'TypeScheme':
...
class TypeEnvironment:
    ...
    def free_variables(self) -> Set[str]:
        """Get all free variables in the environment"""
        vars_set = set()
        for type_scheme in self.bindings.values():
            vars_set.update(type_scheme.type.free_variables())
            vars_set -= set(type_scheme.type_vars)

        if self.parent:
            vars_set.update(self.parent.free_variables())

        return vars_set

    def __str__(self) -> str:
        bindings_str = ", ".join(f"{name}: {scheme}"
...
__all__ = [
    ...
    'list_type', 'dict_type', 'optional_type'
]
