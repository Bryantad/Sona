...
class SonaClass:
    ...
    def __init__(self, name: str, bases: List['SonaClass'] = None,
    ...
    def get_method(self, name: str) -> Optional[Callable]:
        """Find method in class hierarchy using MRO."""
        for cls in self.mro:
            if name in cls.methods:
                return cls.methods[name]
        return None

    def get_static_method(self, name: str) -> Optional[Callable]:
        """Find static method in class hierarchy."""
        for cls in self.mro:
            if name in cls.static_methods:
                return cls.static_methods[name]
        return None

    def get_class_method(self, name: str) -> Optional[Callable]:
        """Find class method in class hierarchy."""
        for cls in self.mro:
            if name in cls.class_methods:
                return cls.class_methods[name]
        return None
    ...
    def create_instance(self, *args, **kwargs) -> 'SonaObject':
...
class SonaObject:
    ...
    def __init__(self, sona_class: SonaClass):
        self._class_ref = sona_class
        
        # Initialize instance variables with defaults
        for name, default in sona_class.instance_variables.items():
            self.__dict__[name] = default

    def get_class(self) -> SonaClass:
        """Get the class of this object."""
        return self._class_ref

    def get_attribute(self, name: str) -> Any:
        """Get an attribute value with proper method binding."""
        ...
        if prop:
            if isinstance(prop, PropertyDescriptor):
        ...
        if class_method:
            return BoundClassMethod(class_method, self._class_ref)

        msg = f"'{self._class_ref.name}' object has no attribute '{name}'"
        raise AttributeError(msg)

    def set_attribute(self, name: str, value: Any):
        """Set an attribute value with property support."""
        prop = self._class_ref.get_property(name)
        if prop and isinstance(prop, PropertyDescriptor):
            prop.set(self, value)
        else:
    ...
    def __repr__(self):
        return f"<{self._class_ref.name} object>"
...
class BoundMethod:
    ...
    def __init__(self, method: Callable, instance: SonaObject):
        self.method = method
    ...
    def __repr__(self):
        return f"<bound method {self.method.__name__} of {self.instance}>"
...
class BoundClassMethod:
    ...
    def __init__(self, method: Callable, cls: SonaClass):
        self.method = method
        self.cls = cls
    ...
    def __repr__(self):
        return f"<bound class method {self.method.__name__} of {self.cls}>"
...
class PropertyDescriptor:
    ...
    def get(self, instance: SonaObject) -> Any:
        if self.getter:
            return self.getter(instance)
        raise AttributeError("Property has no getter")

    def set(self, instance: SonaObject, value: Any):
        if self.setter:
    ...
    def __repr__(self):
        return f"<property getter={self.getter} setter={self.setter}>"
...
class ClassBuilder:
    ...
    def add_base(self, base_class: SonaClass):
        """Add a base class for inheritance."""
    ...
    def add_property(self, name: str, getter: Callable = None,
                     setter: Callable = None):
        """Add a property to the class."""
        self.properties[name] = PropertyDescriptor(getter, setter)
        return self

    def add_instance_variable(self, name: str, default_value: Any = None):
        """Add an instance variable with default value."""
        self.instance_variables[name] = default_value
        return self

    def build(self) -> SonaClass:
        """Build and return the final class."""
        return SonaClass(
            name=self.name,
            bases=self.bases,
...
class InheritanceManager:
    ...
    @staticmethod
    def is_instance(obj: Any, cls: SonaClass) -> bool:
        """Check if object is an instance of a class or its subclasses."""
        if not isinstance(obj, SonaObject):
            return False
    ...
    @staticmethod
    def is_subclass(subclass: SonaClass, baseclass: SonaClass) -> bool:
        """Check if a class is a subclass of another."""
        return baseclass in subclass.mro

    @staticmethod
    def get_super_method(cls: SonaClass, method_name: str) -> Optional[
            Callable]:
        """Get method from superclass for super() calls."""
        # Skip the current class and look in parent classes
        for parent_cls in cls.mro[1:]:
            if method_name in parent_cls.methods:
                return parent_cls.methods[method_name]
        return None
...
def create_class(name: str, bases: List[SonaClass] = None) -> ClassBuilder:
    ...
    for method_name, method_impl in STANDARD_METHODS.items():
        builder.add_method(method_name, method_impl)

    return builder


def super_call(instance: SonaObject, method_name: str, *args,
               **kwargs) -> Any:
    """
    ...
    if super_method:
        return super_method(instance, *args, **kwargs)
    else:
        raise AttributeError(f"super() has no method '{method_name}'")
