...
def integrate_phase2_features(interpreter_class):
    ...
    This function enhances the interpreter with:
    ...
    def handle_phase2_rules(self, tree):
        """Route Phase 2 specific syntax trees to appropriate handlers"""
        
        # Advanced loop constructs
        if tree.data == 'for_in_stmt':
            return self.handle_for_in_loop(tree)
        elif tree.data == 'while_else_stmt':
            return self.handle_while_else_loop(tree)
        elif tree.data == 'loop_until_stmt':
            return self.handle_loop_until(tree)
        
        # Pattern matching
        elif tree.data == 'match_stmt':
            return self.handle_match_statement(tree)
        elif tree.data == 'destructuring_assignment':
            return self.handle_destructuring_assignment(tree)
        
        # Comprehensions
        elif tree.data == 'list_comp':
            return self.handle_list_comprehension(tree)
        elif tree.data == 'dict_comp':
            return self.handle_dict_comprehension(tree)
        
        # Control flow
        elif tree.data == 'break_stmt':
            return self.handle_break_statement(tree)
        elif tree.data == 'continue_stmt':
            return self.handle_continue_statement(tree)
        
        # Object-oriented features (placeholder for future implementation)
        elif tree.data == 'class_def':
            return self.handle_class_definition(tree)
        
        # Fall back to original handler
        return None
    
    def handle_break_statement(self, tree):
        """Handle break statement with optional label"""
        label = str(tree.children[0]) if tree.children else None
        if hasattr(self, 'advanced_loops'):
            self.advanced_loops.loop_control.set_break(label)
        raise RuntimeError("break")  # Use exception for control flow
    
    def handle_continue_statement(self, tree):
        """Handle continue statement with optional label"""
        label = str(tree.children[0]) if tree.children else None
        if hasattr(self, 'advanced_loops'):
            self.advanced_loops.loop_control.set_continue(label)
        raise RuntimeError("continue")  # Use exception for control flow
    
    def handle_dict_comprehension(self, tree):
        """Handle dictionary comprehension"""
        if not hasattr(self, 'advanced_loops'):
            from sona.control.loops import AdvancedLoops
            self.advanced_loops = AdvancedLoops(self)
        
        key_expr = tree.children[0]
        value_expr = tree.children[1]
        for_clauses = [child for child in tree.children[2:] 
                      if child.data == 'for_clause']
        if_clauses = [child for child in tree.children[2:] 
    ...
    def handle_class_definition(self, tree):
        """Placeholder for class definition handling"""
        # This will be implemented when we add OOP features
        class_name = str(tree.children[0])
        print(f"Class definition for '{class_name}' - OOP coming in next phase!")
        return None
    
    # Enhance the visit_tree method to handle Phase 2 constructs
    original_visit_tree = interpreter_class.visit_tree
    
    def enhanced_visit_tree(self, tree):
        """Enhanced visit_tree that handles Phase 2 constructs"""
...
def enable_phase2_features():
    ...
    print("   ✅ Advanced Loop Constructs (for...in, while...else, loop...until)")
