...
class SonaInterpreter(Transformer):
    ...
    def var(self, args):
        ...
        if isinstance(args[0], Token):
            ...
            # First check if the variable exists in any scope (including function parameters)
            for scope in reversed(self.env):
                if name in scope:
    ...
    def func_def(self, args):
        """Handle function definition - this should NOT be called with our new approach"""
        debug(f"!!! UNEXPECTED: func_def method called with {len(args)} args")
        # This should not happen with our new approach
        return "unexpected_func_def_call"
