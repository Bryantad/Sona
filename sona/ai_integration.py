...
if sfm2_path.exists():
    sys.path.insert(0, str(sfm2_path))

class SonaAIBridge:
    """Bridge class for AI-enhanced Sona programming features"""
    
    def __init__(self):
        self.sfm2_available = False
        self.gpt2_available = False
        
        # Try to import SFM-2 components
        try:
            from sfm2.api.app import generate_with_sfm2
            self.generate_with_sfm2 = generate_with_sfm2
            self.sfm2_available = True
            print("✅ SFM-2 integration available")
        except ImportError as e:
            print(f"⚠️ SFM-2 not available: {e}")
        ...
        try:
            ...
            print(f"⚠️ GPT-2 not available: {e}")
    
    def code_completion(self, code_context: str, max_length: int = 100) -> str:
        """Generate code completion suggestions"""
        if self.sfm2_available:
            try:
                return self.generate_with_sfm2(code_context, "completion", max_length)
            except Exception as e:
                print(f"SFM-2 completion failed: {e}")
        
        if self.gpt2_available:
            try:
                return self._gpt2_completion(code_context, max_length)
            except Exception as e:
                print(f"GPT-2 completion failed: {e}")
        
        return f"# AI completion not available for: {code_context[:50]}..."
    
    def _gpt2_completion(self, context: str, max_length: int) -> str:
