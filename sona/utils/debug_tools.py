...
def run_code_with_trace(code, interpreter = None, parser = None): """
    ...
    Args: code (str): The Sona code to execute
    ...
    if not interpreter or not parser: from lark import Lark
        import os
        ...
        if not parser: grammar_path = os.path.join(
    ...
    def traced_func_call(args): # Extract function name and arguments
        name_node = args[0]
        ...
        if (
            len(args) > 1
            and isinstance(args[1], Tree)
            and args[1].data == "args"
        ): passed_args = [
                interpreter.eval_arg(arg) for arg in args[1].children
            ]

        print(
            f"[TRACE] Calling function '{func_name}' with args: {passed_args}"
        )
...
def run_with_profiling(func, *args, **kwargs): """
    ...
    Args: func: The function to run *args: Arguments for the function **kwargs: Keyword arguments for the function
...
def watch_variable(interpreter, var_name): """
    ...
    Args: interpreter (SonaInterpreter): The interpreter instance
        var_name (str): The name of the variable to watch

    Returns: tuple: (found, value, type_name, scope_idx) or None if not found
    """
    if not interpreter or not hasattr(interpreter, 'env'): return None

    # Check all scopes from most local to most global
    for scope_idx, scope in enumerate(reversed(interpreter.env)): depth = (
        len(interpreter.env) - scope_idx - 1
    )
        if var_name in scope: value = scope[var_name]
            return True, value, type(value).__name__, depth

    # Check modules
    if hasattr(interpreter, 'modules') and var_name in interpreter.modules: value = (
        interpreter.modules[var_name]
    )
...
def print_debug_info(interpreter, last_error = None, last_tree = None): """
    ...
    if last_tree: print("\nLast Parse Tree:")
        if hasattr(last_tree, 'pretty'): print(last_tree.pretty())
        else: print(pprint.pformat(last_tree))
    else: print("\nNo parse tree available.")

    print("\nEnv Scope:")
    if interpreter and hasattr(interpreter, 'env'): for scope_idx, scope in enumerate(interpreter.env): if scope_idx = (
        = 0: print("Global scope:")
    )
            else: print(f"Scope {scope_idx}:")
            for name, value in scope.items(): print(f"{name} = {value}")
    else: print("No environment available")
