...
def transpile_command(args):
    ...
    if len(args) < 2:
        print("Usage: sona transpile <file.sona> [--target language] "
              "[--output file.ext]")
        print("Supported languages: python, javascript, typescript, java, "
              "csharp, go, rust")
        return
    ...
    try:
        ...
        if output_file:
            ...
            print(f"✅ Transpiled to {target_language} and saved to "
                  f"{output_file}")
        else:
            print(f"=== Transpiled to {target_language.upper()} ===")
...
def print_help():
    """Print help information for the Sona CLI."""
    ...
    print("\nTranspile Options:")
    print("  --target <language>      Target language (python, javascript, "
...
"""
        ...
        with open(f"{project_name}/main.sona", "w") as f:
            f.write(main_sona)
...
- `main.sona` - Main program entry point
- `README.md` - This file

## Documentation

- [Sona Language Documentation](https://github.com/Bryantad/Sona)
...
def format_command(args):
    ...
    if not args:
        print("Usage: sona format <file.sona> [files...]")
        return
    
    import re
    
    def format_sona_code(code):
        """Basic Sona code formatter"""
        lines = code.split('\n')
        formatted_lines = []
        indent_level = 0
...
def check_command(args):
    """Check Sona code for syntax errors"""
    if not args:
        print("Usage: sona check <file.sona> [files...]")
        return
...
def info_command(args):
    """Show system and Sona environment information"""
    import platform
    import subprocess
    ...
    if sona_vars:
        print(f"🌍 Sona Environment Variables:")
        for k, v in sona_vars.items():
            print(f"  {k} = {v}")
...
def docs_command(args):
    """Open documentation or show quick reference"""
...
🚀 Commands:
  sona file.sona                   // Execute Sona file
...
""")
        ...
        try:
            ...
            print("❌ Could not open browser. Use 'sona docs --offline' for offline help")
...
def main():
    ...
    if "--version" in sys.argv:
        print("Sona Language Version: 0.8.0")
        return
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "clean":
        clean_command(sys.argv[2:])
        return
    
    # Handle docs command
    if len(sys.argv) >= 2 and sys.argv[1] == "docs":
        docs_command(sys.argv[2:])
        return
    
    # Handle run command
    if len(sys.argv) >= 2 and sys.argv[1] == "run":
        # Shift arguments for the runner
        runner_args = sys.argv[2:]
        
        # Check for --cwd option
        cwd = None
        env_vars = {}
        if "--cwd" in runner_args:
            cwd_index = runner_args.index("--cwd")
            if cwd_index + 1 < len(runner_args):
                cwd = runner_args[cwd_index + 1]
                runner_args.pop(cwd_index)  # Remove --cwd
                runner_args.pop(cwd_index)  # Remove path
            
        # Check for --env option
        if "--env" in runner_args:
            env_index = runner_args.index("--env")
            while env_index + 1 < len(runner_args) and "=" in runner_args[env_index + 1]:
        ...
        if runner_args:
            target_file = runner_args[0]
            if os.path.exists(target_file):
                # Check if it's a Sona file
                if target_file.endswith('.sona'):
                    ...
                    try:
                        ...
                        from sona_transpiler import (
                            TranspileOptions, TargetLanguage
                        )
                        ...
                        print(f"❌ Error transpiling {target_file}: {e}")
                        if os.path.exists(temp_py):
                            os.remove(temp_py)
            ...
            print("Usage: sona run <file.py|file.sona> [options]")
