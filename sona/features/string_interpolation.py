...
class StringInterpolationError(Exception):
...
class LiteralPart(InterpolatedStringPart):
    """Literal text part of interpolated string"""
    def __init__(self, text: str):
        self.text = text
    
    def __repr__(self):
        return f"Literal({repr(self.text)})"


class ExpressionPart(InterpolatedStringPart):
    """Expression part of interpolated string"""
    def __init__(self, expression: str, format_spec: str = None):
        self.expression = expression
        self.format_spec = format_spec
    
    def __repr__(self):
        if self.format_spec:
            return f"Expression({self.expression}:{self.format_spec})"
        return f"Expression({self.expression})"


class StringInterpolationParser:
    """Parse f-strings into literal and expression parts"""
    ...
    def parse_f_string(self, f_string_content: str) -> List[
            InterpolatedStringPart]:
        """
        Parse f-string content into parts
        
        Args:
            f_string_content: Content of f-string without f" prefix/suffix
            
        Returns:
            List of InterpolatedStringPart objects
        """
        self.parse_errors.clear()
        parts = []
        last_end = 0
        
        for match in self.INTERPOLATION_PATTERN.finditer(f_string_content):
            start, end = match.span()
            
            # Add literal text before this expression
            if start > last_end:
                literal_text = f_string_content[last_end:start]
                if literal_text:
                    parts.append(LiteralPart(literal_text))
        ...
        if last_end < len(f_string_content):
            remaining_text = f_string_content[last_end:]
            if remaining_text:
                parts.append(LiteralPart(remaining_text))
...
class StringInterpolationEvaluator:
    """Evaluate interpolated strings in Sona context"""
    ...
    def evaluate_f_string(self, f_string_content: str) -> str:
        ...
        Args:
            f_string_content: Content without f" prefix/suffix
              Returns:
            Evaluated string with expressions substituted
        """
        parts = self.parser.parse_f_string(f_string_content)
        
        if self.parser.get_parse_errors():
            errors = "; ".join(self.parser.get_parse_errors())
        ...
        for part in parts:
            ...
            elif isinstance(part, ExpressionPart):
                try:
    ...
    def _evaluate_expression(self, expression: str) -> Any:
        ...
        try:
            # Use Python's eval for now (will be replaced with Sona evaluation)
            # In production, this would use the Sona interpreter's evaluation
            return eval(expression, {}, self._get_current_scope())
        except Exception as e:
            raise StringInterpolationError(f"Cannot evaluate expression: {e}")
    
    def _is_simple_variable(self, expression: str) -> bool:
        """Check if expression is just a simple variable name"""
        return (expression.isidentifier() and
                not any(char in expression for char in '()[]{}.,+-*/'))
    
    def _get_variable_value(self, var_name: str) -> Any:
        """Get variable value from interpreter environment"""
        # Search through environment stack
        for env in reversed(self.interpreter.env):
            if var_name in env:
                return env[var_name]
        
        # Check modules
        if var_name in self.interpreter.modules:
            return self.interpreter.modules[var_name]
        
        raise StringInterpolationError(f"Variable '{var_name}' not found")
    
    def _get_current_scope(self) -> dict:
        """Get current scope as a dictionary for evaluation"""
        scope = {}
        for env in self.interpreter.env:
            scope.update(env)
    ...
    def _format_number(self, value: Union[int, float],
                       format_spec: str) -> str:
        """Format numeric values according to format specification"""
        try:
            # Basic numeric formatting
            if '.' in format_spec:
                # Decimal precision: .2f, .3f, etc.
...
def create_f_string_node(content: str) -> Tree:
    """Create a Tree node for f-string that can be processed by interpreter"""
    return Tree('f_string', [Token('STRING', content)])


class StringInterpolationIntegration:
    """Integration layer for adding string interpolation to Sona interpreter"""
    ...
    def add_to_interpreter(interpreter_class):
        ...
        def handle_f_string(self, node):
            """Handle f-string evaluation in the interpreter"""
            if not hasattr(self, 'string_interpolator'):
                self.string_interpolator = StringInterpolationEvaluator(self)
            ...
            if len(node.children) == 1 and isinstance(node.children[0], Token):
                ...
                f_prefixes = ('f"', "f'")
                quote_suffixes = ('"', "'")
                if (content.startswith(f_prefixes) and
                        content.endswith(quote_suffixes)):
                    content = content[2:-1]
                elif content.startswith(('f"""', "f'''")):
                    content = content[4:-3]
...
def test_string_interpolation():
    ...
    class MockInterpreter:
        def __init__(self):
            self.env = [{"name": "Alice", "age": 30, "score": 85.7}]
            self.modules = {}
    ...
    for f_string, expected in test_cases:
        try:
            result = evaluator.evaluate_f_string(f_string)
            status = "✅" if result == expected else "❌"
            print(f"{status} f\"{f_string}\" -> \"{result}\"")
            if result != expected:
                print(f"   Expected: \"{expected}\"")
        except Exception as e:
            print(f"❌ f\"{f_string}\" -> Error: {e}")
