...
class SonaInterpreter(Transformer):
    ...
    def var(self, args):
        # Simple variable name with optimized parameter handling
        if isinstance(args[0], Token):
            name = str(args[0])
            
            # Debug all available scopes - enhanced for parameter debug
            all_scopes = []
            for i, scope in enumerate(self.env):
                all_scopes.append(f"Scope {i}: {list(scope.keys())}")
            print(f"Looking for variable '{name}' in scopes: {all_scopes}")
            
            # First check if the variable exists in the current function scope (parameters)
            if len(self.env) > 1 and name in self.env[-1]:
                print(f"Found parameter '{name}' = {self.env[-1][name]} in local scope")
                return self.env[-1][name]
            
            # Then check in all other scopes
            for scope in reversed(self.env):
                if name in scope:
                    print(f"Found variable '{name}' = {scope[name]}")
                    return scope[name]
