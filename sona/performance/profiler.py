...
class PerformanceProfiler:
    """Advanced performance profiling for Sona optimization insights"""
    ...
    def record_variable_access(self, var_name: str, scope_level: int, 
                             was_cached: bool = False):
    ...
    def record_optimization(self, opt_type: str, details: Dict[str, Any]):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        for func_name, calls in self.call_times.items():
            total_func_calls = len(calls)
            cached_func_calls = sum(1 for call in calls if call['cached'])
            avg_time = sum(call['time_ms'] for call in calls) / total_func_calls
            
            function_stats[func_name] = {
                'total_calls': total_func_calls,
                'cached_calls': cached_func_calls,
                'cache_hit_rate': (cached_func_calls / total_func_calls * 100) 
                                 if total_func_calls > 0 else 0,
                'avg_time_ms': round(avg_time, 3),
                'total_time_ms': round(sum(call['time_ms'] for call in calls), 3)
            }
        ...
        for event in self.optimization_events:
            if event['type'] == 'optimization':
                optimization_counts[event['optimization_type']] += 1
        
        return {
            'runtime_seconds': round(total_runtime, 3),
            'total_function_calls': total_calls,
            'cached_function_calls': cached_calls,
            'overall_cache_hit_rate': (cached_calls / total_calls * 100) 
            ...
            'total_optimization_events': len([e for e in self.optimization_events 
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        for func_name, calls in self.call_times.items():
            if len(calls) >= 10:
                cached_ratio = sum(1 for call in calls if call['cached']) / len(calls)
                if cached_ratio < 0.5:
        ...
        high_access_vars = [var for var, count in self.variable_accesses.items() 
                           if count >= 20]
        if high_access_vars:
            recommendations.append(
        ...
        if len(self.optimization_events) < 10:
            recommendations.append(
    ...
    def export_detailed_report(self, filename: str):
    ...
    def print_summary_report(self):
        ...
        print(f"Overall Cache Hit Rate: {summary['overall_cache_hit_rate']:.1f}%")
        ...
        for var_name, count in summary['most_accessed_variables'][:5]:
            print(f"  {var_name}: {count} accesses")
