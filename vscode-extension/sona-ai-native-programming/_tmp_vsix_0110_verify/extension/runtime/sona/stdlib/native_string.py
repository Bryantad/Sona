...
def string_reverse(value: Any) -> str:
    return _string.reverse(value)
...
def string_template(text: Any, variables: Optional[Mapping[str, Any]] = None) -> str:
    return _string.template(text, variables or {})
...
# ---------------------------------------------------------------------------
# Export unprefixed aliases
# ---------------------------------------------------------------------------

# Many Sona programs use `string.upper(...)` while older ones used
# `string.string_upper(...)`. Keep both by aliasing `string_*` to `*`.
for _name, _value in list(globals().items()):
    if _name.startswith("string_") and callable(_value):
        _alias = _name[len("string_"):]
        globals().setdefault(_alias, _value)

# Compatibility aliases used by some examples
if "endswith" in globals():
    globals().setdefault("ends_with", globals()["endswith"])
if "startswith" in globals():
    globals().setdefault("starts_with", globals()["startswith"])
