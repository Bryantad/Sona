...
class TestSuite:
    ...
    def run(self):
        ...
        for name, func in self.tests:
            if _only_tests and func not in _only_tests:
                continue
            start = time.time()
            ...
            try:
                # Run setup
                if self.setup_func:
                    _invoke_callable(self.setup_func)
                
                # Run test
                _invoke_callable(func)
                
                # Run teardown
                if self.teardown_func:
                    _invoke_callable(self.teardown_func)
...
def _invoke_callable(func, *args, **kwargs):
    if hasattr(func, "call"):
        return func.call(list(args), dict(kwargs))
    if callable(func):
...
def only(description, func):
    """
    Add test case (BDD-style) and mark it as the only one to run.
    """
    if _current_suite is None:
        raise RuntimeError("Call describe() before only()")

    _current_suite.add_test(description, func)
    _only_tests.add(func)
...
def snapshot(value, name):
    """
    Compare value against a stored snapshot (creates it if missing).

    Snapshots are stored under .sona/snapshots/<name>.snap
    """
    from pathlib import Path
    import json

    base = Path(".sona") / "snapshots"
    base.mkdir(parents=True, exist_ok=True)
    path = base / f"{name}.snap"
