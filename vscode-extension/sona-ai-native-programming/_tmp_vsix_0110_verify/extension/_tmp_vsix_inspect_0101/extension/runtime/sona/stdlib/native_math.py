...
def math_max(*values: Any) -> float:
    # Support calls like math.max(1, 2, 3)
    if len(values) == 1:
        return _math.maximum(_coerce_iterable(values[0]))
    return _math.maximum(values)

def math_maximum(values: Any) -> float:
    return _math.maximum(_coerce_iterable(values))

def math_min(*values: Any) -> float:
    # Support calls like math.min(1, 2, 3)
    if len(values) == 1:
        return _math.minimum(_coerce_iterable(values[0]))
