...
- Keeps the separately installed Python runtime model unchanged.
- Uses the public `python -m sona` CLI entry point for extension commands.
- Documents Cognitive Diagnostics and `sona check --json` for the 0.15.1 runtime.
- Packages the restored primary VSIX tree for manual testing.

## 0.14.1

- Aligns the extension with Sona 0.14.1.
- Keeps the existing command, syntax highlighting, and separately installed Python runtime model unchanged.
- Updates release metadata and package documentation for the Sona-native stdlib foundation.
- Maintains primary VSIX package hygiene for release artifact validation.
