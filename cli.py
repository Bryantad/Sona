...
Commands:
  ...
  sona setup --provider openai   # Configure OpenAI credentials
  sona setup --provider anthropic # Configure Anthropic Claude credentials
  sona setup --provider google   # Configure Google Gemini credentials
  sona setup --provider multi    # Configure multiple providers (recommended)
...
# Local imports
from multi_ai_provider import USER_CONFIG, USER_CONFIG_PATH, _save_user_config
...
def cmd_setup_openai(args):
    """Configure OpenAI provider with user credentials"""
    print("🛡️  Sona Setup: OpenAI Provider")
    print("Enter your OpenAI credentials.")
    print("These will be saved to your user profile directory.")

    api_key = input("OPENAI_API_KEY: ").strip()
    model = input("OPENAI_MODEL (default gpt-4o-mini): ").strip() or "gpt-4o-mini"
    ...
    _save_user_config(cfg)
    print(f"✅ Saved user config to: {USER_CONFIG_PATH}")
    print("You can now run Sona with your OpenAI credentials configured.")


def cmd_setup_anthropic(args):
    """Configure Anthropic Claude provider with user credentials"""
    print("🛡️  Sona Setup: Anthropic Claude Provider")
    print("Enter your Anthropic credentials.")
    print("These will be saved to your user profile directory.")

    api_key = input("ANTHROPIC_API_KEY: ").strip()
    model = input("ANTHROPIC_MODEL (default claude-3-haiku-20240307): ").strip() or "claude-3-haiku-20240307"
    ...
    _save_user_config(cfg)
    print(f"✅ Saved user config to: {USER_CONFIG_PATH}")
    print("You can now run Sona with your Anthropic credentials configured.")


def cmd_setup_google(args):
    """Configure Google Gemini provider with user credentials"""
    print("🛡️  Sona Setup: Google Gemini Provider")
    print("Enter your Google AI credentials.")
    print("These will be saved to your user profile directory.")

    api_key = input("GOOGLE_API_KEY: ").strip()
    model = input("GOOGLE_MODEL (default gemini-pro): ").strip() or "gemini-pro"
    ...
    _save_user_config(cfg)
    print(f"✅ Saved user config to: {USER_CONFIG_PATH}")
    print("You can now run Sona with your Google credentials configured.")


def cmd_setup_multi(args):
    """Configure multiple AI providers at once"""
    print("🛡️  Sona Setup: Multi-Provider Configuration")
    print("Configure multiple AI providers for failover and load balancing.")
    print()
    ...
    # Azure OpenAI
    if input("Configure Azure OpenAI? (y/n): ").strip().lower() == 'y':
        print("Azure OpenAI Configuration:")
        api_key = input("  AZURE_OPENAI_API_KEY: ").strip()
        endpoint = input("  AZURE_OPENAI_ENDPOINT: ").strip()
        deployment = input("  AZURE_OPENAI_DEPLOYMENT_NAME (default gpt-4o-mini): ").strip() or "gpt-4o-mini"
    ...
    # Google
    if input("Configure Google Gemini? (y/n): ").strip().lower() == 'y':
    ...
    if providers:
        ...
        print("You can now run Sona with multi-provider AI support!")
    else:
...
def cmd_info(args):
    ...
    cfg = USER_CONFIG
    if not cfg:
        print("No user config found. Run: sona setup --provider multi")
        return

    providers = []

    # Check Azure OpenAI
    if cfg.get('AZURE_OPENAI_API_KEY') and cfg.get('AZURE_OPENAI_ENDPOINT'):
        print("✅ Azure OpenAI configured:")
        print(f"  Endpoint: {cfg.get('AZURE_OPENAI_ENDPOINT')}")
        print(f"  Deployment: {cfg.get('AZURE_OPENAI_DEPLOYMENT_NAME', 'gpt-4o-mini')}")
        providers.append("Azure OpenAI")

    # Check OpenAI
    if cfg.get('OPENAI_API_KEY'):
        print("✅ OpenAI configured:")
        print(f"  Model: {cfg.get('OPENAI_MODEL', 'gpt-4o-mini')}")
        providers.append("OpenAI")

    # Check Anthropic
    if cfg.get('ANTHROPIC_API_KEY'):
        print("✅ Anthropic Claude configured:")
        print(f"  Model: {cfg.get('ANTHROPIC_MODEL', 'claude-3-haiku-20240307')}")
        providers.append("Anthropic Claude")

    # Check Google
    if cfg.get('GOOGLE_API_KEY'):
        print("✅ Google Gemini configured:")
        print(f"  Model: {cfg.get('GOOGLE_MODEL', 'gemini-pro')}")
        providers.append("Google Gemini")

    if providers:
        print(f"\n🎯 Multi-provider AI active: {', '.join(providers)}")
        print("(API keys are kept private and not displayed)")
    else:
        print("❌ No providers configured. Run: sona setup --provider multi")
        ...
        try:
            ...
            if 'function sona' in content:
                ...
                with open(profile, 'a', encoding='utf-8') as f:
                    f.write('\n' + func)
                print("✅ Added 'sona' function to PowerShell profile")
        except Exception as e:
...
def main(argv=None):
    ...
    p_setup.add_argument('--provider', choices=['azure', 'openai', 'anthropic', 'google', 'multi'], default='multi')
    ...
    if args.command == 'setup':
        provider = getattr(args, 'provider', 'multi')
        if provider == 'azure':
            cmd_setup_azure(args)
        elif provider == 'openai':
            cmd_setup_openai(args)
        elif provider == 'anthropic':
            cmd_setup_anthropic(args)
        elif provider == 'google':
            cmd_setup_google(args)
        elif provider == 'multi':
            cmd_setup_multi(args)
        else:
            cmd_setup_azure(args)  # fallback
