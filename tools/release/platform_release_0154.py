#!/usr/bin/env python3
"""Build, inspect, verify, and assemble Sona 0.15.4 platform artifacts.
...
def _build_test_kit(destination: Path, epoch: int) -> dict[str, Any]:
    ...
    entries = [
        ...
        (
            f"{prefix}/hello.sona",
            b'print("Sona 0.15.4 platform test kit: OK")\n',
            0o644,
        ),
    ]
...
def _native_smoke(binary: Path) -> dict[str, Any]:
    expected = "Sona 0.15.4 cross-platform Native Core: OK"
    with tempfile.TemporaryDirectory(prefix="sona-0154-native-smoke-") as raw:
        root = Path(raw)
...
def _cargo_build(spec: NativeSpec, output_dir: Path) -> tuple[Path, dict[str, Any]]:
    ...
    binary = target_dir / spec.rust_target / "release" / spec.archive_member
...
def _python_guardian_smoke(
    ...
    expected = "Sona 0.15.4 platform Python and Guardian: OK"
    with tempfile.TemporaryDirectory(prefix="sona-0154-platform-verify-") as raw:
        root = Path(raw)
        source = root / "hello.sona"
        source.write_text(f'print("{expected}")\n', encoding="ascii", newline="\n")
...
def command_verify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="sona-0154-native-extract-") as raw:
...
def command_assemble(args: argparse.Namespace) -> dict[str, Any]:
    ...
    manifest = {
        ...
        "mobile": {
            "android": "experimental compile/Python-only evaluation; not a 0.15.4 release target",
