...
def init_project(root: Path, *, name: str | None = None, version: str = "0.10.0") -> Path:
    path = _manifest_path(root)
    if path.exists():
        return path
...
def generate_catalog(stdlib_path: Path | None = None, output: Path | None = None) -> dict[str, Any]:
    ...
    catalog: dict[str, Any] = {
        "version": manifest.get("version", "0.10.0"),
...
def build_parser(prog: str = "spm") -> argparse.ArgumentParser:
    ...
    p_init.add_argument("--version", default="0.10.0", help="Project version")
