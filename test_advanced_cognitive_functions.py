...
class AdvancedSonaInterpreter(EnhancedSonaInterpreter):
    ...
    def _is_simple_statement(self, code: str) -> bool:
        ...
        # If code contains newlines, it's not a simple statement
        if '\n' in code:
            return False
        ...
        # Check if it's a single-line function call (ends with parentheses)
        if '(' in code and code.rstrip().endswith(')') and '\n' not in code:
            return True
